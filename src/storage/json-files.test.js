...
const { OPEN_CONSUMTION, CLOSED_CONSUMTION } = require('../fixtures/consumptions')
...
test('save and read consumption', async () => {
  ...
  const firstConsumption = await storage.readMonthlyConsumption(OPEN_CONSUMTION.year, OPEN_CONSUMTION.month)
  expect(firstConsumption).toEqual(OPEN_CONSUMTION)

  const secondConsumption = await storage.readMonthlyConsumption(CLOSED_CONSUMTION.year, CLOSED_CONSUMTION.month)
  expect(secondConsumption).toEqual(CLOSED_CONSUMTION)

  // Override exsiting files
  const newLastUpdated = "2026-01-13T12:33:27.751Z"
  const openConsumtionUpdated = {... OPEN_CONSUMTION, lastUpdated: newLastUpdated}
  await storage.saveMonthlyConsumption(openConsumtionUpdated)
  const updatedConsumption = await storage.readMonthlyConsumption(OPEN_CONSUMTION.year, OPEN_CONSUMTION.month)
